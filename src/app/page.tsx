...
import { Check, CreditCard, Settings2, Zap } from 'lucide-react'
...
export default async function Home() {
  ...
  const steps = [
    { icon: CreditCard, titleKey: 'step1Title', descKey: 'step1Desc' },
    { icon: Settings2, titleKey: 'step2Title', descKey: 'step2Desc' },
    { icon: Zap, titleKey: 'step3Title', descKey: 'step3Desc' },
  ] as const
          ...
          <div className="mx-auto flex min-h-[calc(100vh-12rem)] max-w-content items-center justify-center text-center">
            <div className="max-w-3xl">
              ...
              <div className="mx-auto flex max-w-sm flex-col justify-center gap-3 sm:max-w-none sm:flex-row">
                <Link href="/register" className="btn-primary sm:w-auto sm:px-8">
                  {tc('getStarted')}
                </Link>
                <Link href="/login" className="btn-secondary sm:w-auto sm:px-8">
                  {tc('signIn')}
                </Link>
              </div>
            </div>
          ...
          <div className="max-w-content mx-auto px-6 md:px-12">
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {steps.map((item) => {
                ...
                return (
                  <div key={item.titleKey} className="brand-card flex h-full gap-4 items-start">
                    <div className="bg-emerald-light dark:bg-emerald/20 w-9 h-9 rounded-md flex items-center justify-center flex-shrink-0">
